...
try:
    ...
    image_model = None
    image_processor = None
    print(f"Error loading image model: {e}. Make sure 'transformers' and 'torch' are installed.")
...
def extract_faces_from_video(video_path, start_time=0, duration=2, num_frames=TIME_STEPS):
    global detector
    if detector is None: raise Exception("MTCNN detector is not initialized.")
...
def run_image_detection_vit(image_path):
    ...
    prediction_label = image_model.config.id2label[predicted_class_idx]
...
def analyze_video_api():
    ...
    try:
        ...
        predictions = video_model.predict(video_array, verbose=0)
